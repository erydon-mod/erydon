package com.oliver.erydon.loot;
...
public final class ErydonLoot {
    ...
    public static void init() {
        LootTableEvents.REPLACE.register((resourceManager, lootManager, id, original, source) -> {
            // Only touch our own loot tables
            ...
            // If there *is* a real table (from our JSON or a datapack), don't replace it
            if (original != LootTable.EMPTY) {
                return null;
            }

            // Derive the block id: erydon:blocks/aganite_block -> erydon:aganite_block
            String blockPath = id.getPath().substring("blocks/".length());
            Identifier blockId = new Identifier(Erydon.MOD_ID, blockPath);
            Block block = Registries.BLOCK.get(blockId);

            // If somehow there is no such block, bail out
            if (block == Blocks.AIR) {
                return null;
            }
