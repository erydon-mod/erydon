...
public final class ColumnModelLoadingPlugin implements ModelLoadingPlugin {
    ...
    public void onInitializeModelLoader(Context pluginContext) {
        ...
        pluginContext.modifyModelAfterBake().register((model, context) -> {
            ...
            if (!Erydon.MOD_ID.equals(mid.getNamespace()) || !ErydonModelFamilyIndex.isColumnBlock(mid.getPath())) {
                return model;
            }

            return cache.wrap(mid, model, ColumnBakedModel::new,
                    ErydonLoadProfiler.customWrapObserver("columns", "inventory".equals(mid.getVariant())));
        });
    }

    private static List<Identifier> extraModels() {
        List<Identifier> models = new ArrayList<>();
        for (String blockPath : ErydonModelFamilyIndex.get().columns()) {
