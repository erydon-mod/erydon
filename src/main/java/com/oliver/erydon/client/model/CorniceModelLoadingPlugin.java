...
public final class CorniceModelLoadingPlugin implements ModelLoadingPlugin {
    ...
    public void onInitializeModelLoader(Context pluginContext) {
        ...
        pluginContext.modifyModelAfterBake().register((model, context) -> {
            ...
            if (!Erydon.MOD_ID.equals(mid.getNamespace()) || !ErydonModelFamilyIndex.isCorniceBlock(mid.getPath())) {
                return model;
            }

            return cache.wrap(mid, model, CorniceBakedModel::new,
                    ErydonLoadProfiler.customWrapObserver("cornices", "inventory".equals(mid.getVariant())));
        });
    }

    private static List<Identifier> extraModels() {
        List<Identifier> models = new ArrayList<>();
        for (String blockPath : ErydonModelFamilyIndex.get().cornices()) {
